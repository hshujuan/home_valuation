"""Build the technical summary PDF from the maintained public Markdown."""
...
#text(size: 9pt, fill: muted)[
  A concise, standalone synthesis. Maintained data contracts, generated
  result tables, and source verification limits accompany it in the repository.
]
#pagebreak()
...
#set document(
  title: "Home Valuation and Two-sided Causal Decisions: Technical Summary",
...
#set page(
  ...
  footer: context align(right)[
    #text(size: 8pt, fill: muted)[
      Technical summary / Synthetic evidence
      #h(1em)
...
def assemble(build):
    ...
    output_pdf = ROOT / "docs" / "design_implementation_summary.pdf"
    ...
    for key, title, relative in sources:
        ...
        for block in blocks:
            if block.get("t") == "Table":
                all_blocks.append(readable_table(block))
            else:
...
def main():
    ...
    build = ROOT / "outputs" / "pdf_summary"
    build.mkdir(parents=True, exist_ok=True)
    ast, hashes = assemble(build)
    (build / "book.json").write_text(json.dumps(ast, ensure_ascii=False), encoding="utf-8")
    body = pandoc(json.dumps(ast), "json", "typst")
    content = STYLE + SUMMARY_COVER.replace("SNAPSHOT_DATE", args.date.isoformat()) + body
    source = build / "book.typ"
    source.write_text(content, encoding="utf-8")
    candidate = build / "design_implementation_summary.pdf"
    typst.compile(str(source), output=str(candidate), root=str(build))
    checks = verify_pdf(candidate)
    output = ROOT / "docs" / candidate.name
    shutil.copyfile(candidate, output)
    manifest = {
        "snapshot_date": args.date.isoformat(), "edition": "summary",
